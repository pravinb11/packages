...
#include "control_planner_interface/msg/path.hpp"
...
namespace interface {
    ...
    public:
        ...
        // PCIManager(const rclcpp::Node &nh, const rclcpp::Node &nh_private) :
        PCIManager(const std::shared_ptr<rclcpp::Node> nh, const std::shared_ptr<rclcpp::Node> nh_private) :
                nh_(nh), nh_private_(nh_private),
                pci_status_(PCIStatus::kReady), force_stop_(false) {

            // trajectory_vis_pub_ = nh_.advertise<visualization_msgs::msg::MarkerArray>(
            //         "pci_command_trajectory_vis", 10);
            trajectory_vis_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>("pci_command_trajectory_vis", 10);
        ...
        virtual bool goToWaypoint(const geometry_msgs::msg::Pose &pose) = 0;
        ...
        virtual bool executePath(const std::vector<control_planner_interface::msg::Path> &path_segments,
                                 std::vector<geometry_msgs::msg::Pose> &modified_path) = 0;
        ...
        virtual void setCurrentPose(const geometry_msgs::msg::Pose &pose) = 0;
