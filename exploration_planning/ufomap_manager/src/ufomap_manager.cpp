...
namespace ufomap_manager {
    ...
    // UFOMapManager::UFOMapManager(rclcpp::Node &nh, rclcpp::Node &nh_private)
    // UFOMapManager::UFOMapManager(const rclcpp::Node::SharedPtr& nh, const rclcpp::Node::SharedPtr& nh_private)
    UFOMapManager::UFOMapManager(const rclcpp::Node::SharedPtr& nh)
            : nh_(nh),
        ...
        cloud_sub_ = nh->create_subscription<sensor_msgs::msg::PointCloud2>("point_cloud", 10, std::bind(&UFOMapManager::insertCloudCallback, this, std::placeholders::_1));

        if (0 < pub_rate_) {
            // map_pub_ = nh_private.advertise<ufomap_msgs::UFOMapStamped>(
            //         "map", 10, nh_private.param("map_latch", false));
            // map_pub_ = nh_private->create_publisher<ufomap_msgs::msg::UFOMapStamped>("map", 10);
            map_pub_ = nh->create_publisher<ufomap_msgs::msg::UFOMapStamped>("map", 10);
            // cloud_pub_ = nh_private.advertise<sensor_msgs::msg::PointCloud2>(
            //         "map_cloud", 10, nh_private.param("map_cloud_latch", false));
            // cloud_pub_ = nh_private->create_publisher<sensor_msgs::msg::PointCloud2>("map_cloud", 10);
            cloud_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("map_cloud", 10);
            // pub_timer_ = nh_private.createTimer(rclcpp::Rate(pub_rate_), &UFOMapManager::timerCallback, this);
            // pub_timer_ = nh_private->create_wall_timer(std::chrono::milliseconds(100), std::bind(&UFOMapManager::timerCallback, this));
            pub_timer_ = nh->create_wall_timer(std::chrono::milliseconds(100), std::bind(&UFOMapManager::timerCallback, this));
    ...
    void UFOMapManager::insertCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {
        ...
        tf2_ros::Buffer tf_buffer_(nh_->get_clock());
        ...
        try {
            ...
            RCLCPP_WARN_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Something went wrong: %s", ex.what());
            return;
        }
        ...
        double integration_time = (nh_->get_clock()->now() - start_time).seconds();
        ...
        if (clear_robot_) {
            ...
            try {
                ...
                transform = ufomap_ros::rosToUfo(
                        tf_buffer_.lookupTransform(frame_id_, robot_frame_id_, msg->header.stamp,
                                                   rclcpp::Duration::from_seconds(1.0)).transform);
            } catch (tf2::TransformException &ex) {
                // ROS_WARN_THROTTLE(1, "%s", ex.what());
                RCLCPP_WARN_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Something went wrong: %s", ex.what());
                return;
