...
namespace ufomap_manager {
    ...
    // FrontierManager::FrontierManager(const rclcpp::Node &nh, const rclcpp::Node &nh_private) :
    // FrontierManager::FrontierManager(const std::shared_ptr<rclcpp::Node> nh, const std::shared_ptr<rclcpp::Node> nh_private) :
    FrontierManager::FrontierManager(const std::shared_ptr<rclcpp::Node> nh) :
        ...
        fout << "record the known space size, starting at " << start_time.seconds() << "\n" << "now time" << "\t"
        ...
        // point_cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::PointCloud2>(nh_, "point_cloud", 1));
        point_cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::PointCloud2>(nh_, "point_cloud", rmw_qos_profile_sensor_data));
        // odom_sub_.reset(new message_filters::Subscriber<nav_msgs::msg::Odometry>(nh_, "sensor_odometry", 1000));
        odom_sub_.reset(new message_filters::Subscriber<nav_msgs::msg::Odometry>(nh_, "sensor_odometry", rmw_qos_profile_sensor_data));
        ...
        sync_point_cloud_odom_->registerCallback(std::bind(&FrontierManager::pointCloudOdomCallback, this, std::placeholders::_1, std::placeholders::_2));

        voxel_filter_.setLeafSize(scan_voxel_size_, scan_voxel_size_, scan_voxel_size_);

        // write_ufomap_timer_ = nh_private_.createTimer(rclcpp::Duration(2.0), &FrontierManager::writeUfomapCallback, this);
        // write_ufomap_timer_ = this->create_wall_timer(std::chrono::seconds(2), std::bind(&FrontierManager::writeUfomapCallback, this));
        write_ufomap_timer_ = nh_->create_wall_timer(std::chrono::seconds(2), std::bind(&FrontierManager::writeUfomapCallback, this));

        if (0 < pub_rate_) {
            // map_pub_ = nh_private_.advertise<ufomap_msgs::UFOMapStamped>("ufomap", 1);
            // map_pub_ = this->create_publisher<ufomap_msgs::msg::UFOMapStamped>("ufomap", 1);
            map_pub_ = nh_->create_publisher<ufomap_msgs::msg::UFOMapStamped>("ufomap", 1);
            // cloud_pub_ = nh_private_.advertise<sensor_msgs::msg::PointCloud2>("ufomap_cloud", 1);
            // cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("ufomap_cloud", 1);
            ...
            // map_and_frontiers_pub_ = this->create_publisher<ufomap_manager::msg::UfomapWithFrontiers>("ufomap_and_frontiers", 1);
            map_and_frontiers_pub_ = nh_->create_publisher<ufomap_manager::msg::UfomapWithFrontiers>("ufomap_and_frontiers", 1);

            // pub_timer_ = nh_private_.createTimer(rclcpp::Rate(pub_rate_), &FrontierManager::ufomapPublishTimer, this);
            // pub_timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FrontierManager::ufomapPublishTimer, this));
            pub_timer_ = nh_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FrontierManager::ufomapPublishTimer, this));
            // frontier_timer_ = nh_private_.createTimer(rclcpp::Rate(pub_rate_), &FrontierManager::frontierCallback, this);
            // frontier_timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FrontierManager::frontierCallback, this));
            frontier_timer_ = nh_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FrontierManager::frontierCallback, this));
        }

        // local_frontiers_pub_ = nh_private_.advertise<visualization_msgs::msg::MarkerArray>("local_frontier_marker", 1);
        // local_frontiers_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("local_frontier_marker", 1);
        local_frontiers_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>("local_frontier_marker", 1);
        // global_frontiers_pub_ = nh_private_.advertise<visualization_msgs::msg::MarkerArray>("global_frontier_marker", 1);
        // global_frontiers_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("global_frontier_marker", 1);
        global_frontiers_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>("global_frontier_marker", 1);
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "frontier_manager construct finish");
    }
    ...
    void FrontierManager::setParametersFromROS() {
        ...
        if (!nh_->get_parameter(ns + "/frame_id", frame_id_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No frame_id specified. Looking for %s. Default is 'map'.",
                     (ns + "/frame_id").c_str());
        }

        robot_base_frame_id_ = "base_link";
        // if (!ros::param::get(ns + "/robot_base_frame_id", robot_base_frame_id_)) {
        if (!nh_->get_parameter(ns + "/robot_base_frame_id", robot_base_frame_id_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No robot_base_frame_id specified. Looking for %s. Default is 'base_link'.",
        ...
        if (!nh_->get_parameter(ns + "/max_range", max_range_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No max_range specified. Looking for %s. Default is '7'.",
                     (ns + "/max_range").c_str());
        }
        ...
        if (!nh_->get_parameter(ns + "/insert_discrete", insert_discrete_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No insert_discrete specified. Looking for %s. Default is 'false'.",
        ...
        if (!nh_->get_parameter(ns + "/clear_robot_enabled", clear_robot_enabled_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No clear_robot_enabled_ specified. Looking for %s. Default is 'true'.",
        ...
        if (!nh_->get_parameter(ns + "/robot_bottom", robot_bottom_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No robot_bottom specified. Looking for %s. Default is '0 m'.",
        ...
        if (!nh_->get_parameter(ns + "/frontier_depth", frontier_depth_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No frontier_depth specified. Looking for %s. Default is '0'.",
        ...
        if (!nh_->get_parameter(ns + "/scan_voxel_size", scan_voxel_size_)) {
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "No scan_voxel_size specified. Looking for %s. Default is '0.1'.",
                     (ns + "/scan_voxel_size").c_str());
        }

    }

    void FrontierManager::pointCloudOdomCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &scan,
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "down size the input point cloud..");
        ...
        map_mutex_.lock();
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "insert cloud");
        insert_num++;
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "finish insert cloud, size is %zu,spend time is %f ", downsize_cloud.size(),
                 // (insert_end_time - insert_start_time).toSec());
                 (insert_end_time - insert_start_time).seconds());
        // sum_insert_time = sum_insert_time + (insert_end_time - insert_start_time).toSec();
        sum_insert_time = sum_insert_time + (insert_end_time - insert_start_time).seconds();
        ...
        // fout << insert_start_time << "\t" << insert_end_time << "\t" << (insert_end_time - insert_start_time).seconds()
        fout << insert_start_time.seconds() << "\t" << insert_end_time.seconds() << "\t" << (insert_end_time - insert_start_time).seconds()
             << "\t" << insert_num << "\t" << sum_insert_time / insert_num << "s \t" << std::endl;
        fout.close();

        try {
            // geometry_msgs::msg::TransformStamped T_s_b = tf_buffer_.lookupTransform(sensor_frame_id_, robot_base_frame_id_,
            //                                                                    odom->header.stamp);
            geometry_msgs::msg::TransformStamped T_s_b = tf_buffer_->lookupTransform(sensor_frame_id_, robot_base_frame_id_, odom->header.stamp);
            ...
            RCLCPP_WARN(rclcpp::get_logger("UfomapManager"), "ufomap with frontier current pose acquire---      %s ", ex.what());
        }
        map_mutex_.unlock();
    }

    // void FrontierManager::ufomapPublishTimer(const rclcpp::TimerEvent &event) {
    void FrontierManager::ufomapPublishTimer() {
        if (insert_num > 1)
            map_.prune();
    ...
    // void FrontierManager::frontierCallback(const rclcpp::TimerEvent &event) {
    void FrontierManager::frontierCallback() {
        ...
        global_frontiers_pub_->publish(global_frontier_cells);
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "global frontiers marker published, size is %zu", global_frontier_cells.markers[0].points.size());
        ...
        local_frontiers_pub_->publish(local_frontier_cells);
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "local frontiers marker published, size is %zu", local_frontier_cells.markers[0].points.size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "frontier map published.");
    }

    void FrontierManager::frontierSearch() {

        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "start frontier update..");
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "start find new frontiers..");
    ...
    // void FrontierManager::writeUfomapCallback(const rclcpp::TimerEvent &event) {
    void FrontierManager::writeUfomapCallback() {
    ...
    void FrontierManager::findLocalFrontier() {
        ...
        RCLCPP_INFO(nh_->get_logger(), "local frontier cells find");
        changed_cell_codes_.clear();
    }

    void FrontierManager::updateGlobalFrontier() {
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "start update global frontier, old frontiers size is %zu", global_frontier_cells_.size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "old global_frontiers detection finish, remain frontiers size is %zu", global_frontiers.size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "global frontier updated");
    }
    ...
    void FrontierManager::knownCellOutput() {
        ...
        auto current_time = nh_->get_clock()->now();
        ...
        fout << current_time.seconds() << "\t" << (current_time - start_time).seconds() << "\t" << known_plane_cell_num_
             << "\t" << static_cast<double>(known_plane_cell_num_) * map_.getResolution() * map_.getResolution()
             << std::endl;
        fout.close();
        RCLCPP_INFO(rclcpp::get_logger("UfomapManager"), "known cell num is %lu", known_plane_cell_num_);
    }
    ...
    void
    FrontierManager::generateMarkerArray(const std::string &tf_frame, visualization_msgs::msg::MarkerArray *frontier_cells,
