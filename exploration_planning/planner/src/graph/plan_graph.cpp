...
namespace graph {
    ...
    bool PlanGraph::getShortestPath(const int &start_v_id, 
                                    const int &end_v_id, 
                                    ...
                                    utils::Point3DQueue &shortest_path) const
    {
        ...
        if(vertexIndexInRange(start_v_id) && vertexIndexInRange(end_v_id))
            ...
            {
                std::cout << "v_" << start_v_id << " = v_" << end_v_id << " in G!" << std::endl;
            ...
            else
            {
                ...
                {
                    std::cout << "A* (v_" << start_v_id << ", v_" << end_v_id << ") in G!" << std::endl;
        ...
        {
            std::cout << "v_" << start_v_id << " or v_" << end_v_id << " is not in G!" << std::endl;

            return false;
        }
    }

    bool PlanGraph::A_star_search(const int &start_v_id, 
                                    const int &end_v_id, 
                                    vector<int> &waypoint_ids) const
    {
